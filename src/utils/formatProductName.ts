import type { ProductWithRelations } from "@/types/products/product.relations";

export function formatProductName(product: ProductWithRelations) {
  const categoria = product.categoria;

  const m = product.materiales?.[0];

  if (!m) return categoria;

  const partes: string[] = [];

  // Evitar repetir la categoría si el tipo ya la contiene
  const tipo = m.tipo ? m.tipo : "";
  if (tipo && !tipo.toLowerCase().includes(categoria.toLowerCase())) {
    partes.push(tipo);
  }

  if (m.ancho_cm && m.largo_cm && m.ancho_cm > 0 && m.largo_cm > 0) {
    partes.push(`${m.ancho_cm}x${m.largo_cm}`);
  }

  if (m.gramaje_g && m.gramaje_g > 0) {
    partes.push(`${m.gramaje_g}g`);
  }

  if (m.calibre && m.calibre > 0) {
    partes.push(`Cal ${m.calibre}`);
  }

  if (m.unidad_medida) {
    partes.push(m.unidad_medida);
  }

  const detalle = partes.filter(Boolean).join(" ").trim();

  // Si el detalle ya incluye la categoría, no la repetimos
  if (detalle.toLowerCase().includes(categoria.toLowerCase())) {
    return detalle;
  }

  return detalle.length > 0 ? `${categoria} ${detalle}` : categoria;
}